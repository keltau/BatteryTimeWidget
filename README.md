...
- App-private SQLite schema version 2 with WAL; migration adds charge tables without changing existing percentage data. One executor serializes database work and file transfers off the UI thread. An event's percentage and charge updates commit in one transaction.
